...
ratcliff_obershelp(const char *begin1, const char *end1, const char *begin2, const char *end2)
{
	...
	if (!length)
		return 0;
	return ratcliff_obershelp(begin1, start1, begin2, start2) + length + ratcliff_obershelp(start1 + length, end1, start2 + length, end2);
...
String_ratcliff(VALUE self, VALUE s)
{
	...
	if(RSTRING(self)->len == 1 && RSTRING(s)->len == 1)
		result = RSTRING(self)->ptr[0] == RSTRING(s)->ptr[0] ? 1.0 : 0.0;
	else
		result = 2.0 * ratcliff_obershelp(RSTRING(self)->ptr, RSTRING(self)->ptr+RSTRING(self)->len, RSTRING(s)->ptr, RSTRING(s)->ptr+RSTRING(s)->len) / (RSTRING(self)->len + RSTRING(s)->len);
